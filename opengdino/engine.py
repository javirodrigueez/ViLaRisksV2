...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        ...
        if args.save_results:
            ...
            # if args.save_bboxes:
            #     print('hola')
            #     f = open('risks_eval/results.txt', 'a')
            # else:
            #     print('maaaaaal')
            for i, (tgt, res) in enumerate(zip(targets, results)):
                ...
                _res_id = tgt['image_id'].expand(res['scores'].shape[0])
                _res_bbox = res['boxes']
                _res_prob = res['scores']
                _res_label = res['labels']
                #print("res_bbox:", _res_bbox.shape)
                #print("res_prob:", _res_prob.shape)
                #print("res_label:", _res_label.shape)
                res_info = torch.cat((_res_bbox, _res_prob.unsqueeze(-1), _res_label.unsqueeze(-1), _res_id.unsqueeze(-1)), 1)
       
                # if args.save_bboxes:
                #     f.write(f'{tgt['image_id'].item()}, {_res_label}, {_res_prob}\n')
