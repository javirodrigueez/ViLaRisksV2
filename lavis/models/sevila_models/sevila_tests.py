...
@registry.register_model("sevila_tests")
class SeViLATests(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        use_vit=True, use_optical_flow=False, device='cpu', total_frames=8, use_qformer_clf=False):
        """
        ...
        self.use_qformer_clf = use_qformer_clf
        self.ANS_MAPPING = {'A':0, 'B':1, 'C':2, 'D':3, 'E':4, 'F':5, 'G':6, 'H':7, 'I':8, 'J':9, 'K':10, 'L':11, 'M':12, 'N':13, 'O':14, 'P':15, 'Q':16, 'R':17, 'S':18, 'T':19, 'U':20, 'V':21, 'W':22, 'X':23, 'Y':24, 'Z':25, 'AA':26, 'BB':27, 'CC':28, 'DD':29, 'EE':30, 'FF':31, 'GG':32}
        ...
        # Optical flow projection layer
        if self.use_optical_flow:
            self.of_proj = nn.Linear(4096, 1408)    # optical flow size, vision size

        # Q-Former for Answerer
        self.Qformer, self.query_tokens = self.init_Qformer(
        num_query_token, 1408) # self.visual_encoder.num_features
        ...
        # MODIFIED: Options expanded from 5 to 32 (A-g)
        #answer_id = [71, 272, 205, 309, 262, 377, 350, 454, 27, 446, 480, 301, 283, 445, 411, 276, 1593, 391, 180, 332, 412, 584, 549, 4, 476, 1027, 9, 115, 75, 26, 15, 89, 122]
        # Options expadned from 32 to 5 (A-GG)
        answer_id = [71, 272, 205, 309, 262, 377, 350, 454, 27, 446, 480, 301, 283, 445, 411, 276, 1593, 391, 180, 332, 412, 584, 549, 4, 476, 1027, 5498, 7640, 2823, 11253, 5080, 9089, 21320]
        ...
        self.frame_num = frame_num  # Take in mind that in case of not using localizer this should be frame_num = total_frames
        ...
        # Q-Former classifier
        if use_qformer_clf:
            clf_hidden_size = self.Qformer.config.hidden_size // 2
            self.qformer_clf = nn.Sequential(
                nn.Linear(self.Qformer.config.hidden_size, clf_hidden_size),
                nn.ReLU(),
                nn.Linear(clf_hidden_size, self.answer_num),
            )
    ...
    def forward(self, samples,
        ...
        of_features = samples['of_features']
        if self.use_vit:
            image = samples["video"]
            ...
            b = len(samples['features_file'])
            t = self.total_frames
            c = 3
            w, h = self.img_size, self.img_size
            data = []
            for i in samples['features_file']:
                embeds = np.load(i)
                embeds = (embeds[0::4] + embeds[1::4] + embeds[2::4] + embeds[3::4]) / 2.0
                data.append(embeds)
            image_embeds = torch.tensor(np.stack(data, axis=0)).to(self.dev)
            _, _, n_emb, c_emb = image_embeds.shape
            image_embeds = image_embeds.reshape(-1, n_emb, c_emb)
        _, n, _ = image_embeds.shape
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(self.dev) # bt n c
        ...
        if 'train_loc' in self.task:
            ...
            with torch.no_grad():
                ...
                atts_t5_qa = torch.ones(inputs_t5_qa.size()[:-1], dtype=torch.long).to(self.dev)
                ...
                with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                    ...
                    input_tokens_qa = self.t5_tokenizer(
                        ...
                        max_length=self.max_txt_len, return_tensors="pt").to(self.dev)
            ...
            atts_t5_loc = torch.ones(inputs_t5_loc.size()[:-1], dtype=torch.long).to(self.dev)
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                frame_prefix = self.t5_tokenizer(
                    self.frame_prefix, padding="longest", add_special_tokens=False,
                    truncation=True, max_length=self.max_txt_len, return_tensors="pt").to(self.dev) 
                frame_prefix_id = torch.repeat_interleave(frame_prefix.input_ids, b*t, 0)
                frame_prefix_mask = torch.repeat_interleave(frame_prefix.attention_mask, b*t, 0)
                frame_predix_embed = self.t5_model.encoder.embed_tokens(frame_prefix_id)
        ...
        # Finetune answerer with localizer
            ...
            ##  project and add optical flow to rgb features
            if self.use_optical_flow:
                of_features = of_features.reshape(b*t, -1, 4096)
                of_features_proj = self.of_proj(of_features)
                image_embeds = torch.cat([image_embeds, of_features_proj], dim=1)
                image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(self.dev)
            ...
            image_atts = torch.ones(select_frames.size()[:-1], dtype=torch.long).to(self.dev) # bt n c
            ...
            ## end forwarding with clf
            if self.use_qformer_clf:
                ## get numerical answer values
                answer = torch.tensor([self.ANS_MAPPING[a.split()[1]] for a in answer]).to(self.dev)
                inputs_clf = query_output_qa.last_hidden_state
                inputs_clf = inputs_clf.reshape(b, t, inputs_clf.shape[-2], -1)
                ## predict
                outputs = self.qformer_clf(inputs_clf)
                loss = nn.CrossEntropyLoss()(outputs, answer)
                return {"loss": loss}
        ...
        # finetune answerer with random frames
            ...
            if self.use_vit:
                image_embeds = self.ln_vision(image_embeds) # TODO: If you finally extract features this should be put outside if
            
            ##  project and add optical flow to rgb features    #TODO: La agregación de OF está mal hecha, tienes que agregarlo a token CLS de Vit. También tiene que añadirse en metodo generate()
            if self.use_optical_flow:
                of_features = of_features.reshape(b*t, -1, 4096)
                of_features_proj = self.of_proj(of_features)
                image_embeds = torch.cat([image_embeds, of_features_proj], dim=1)
        ...
        if 'loc' not in self.task or 'uni_eval' in self.task:
            if self.use_vit:       
                ...
                with torch.cuda.amp.autocast(enabled=(self.device != torch.device("cpu"))):
                    image_embeds = self.ln_vision(self.visual_encoder(image)) # bt, n, c    # TODO: If you finally extract features this should be put outside if
            else:
                b = len(samples['features_file'])
                t = self.frame_num
                c = 3
    ...
    def generate_demo(self,
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            outputs_qa = self.t5_model.generate(
                    ...
                    output_hidden_states=True, output_scores=True)
            pred_logits_qa = outputs_qa.scores[1]
            pred_logits_qa = pred_logits_qa[:, self.answer_id] # b, 5 (5 is the original, now has been expanded to 32)
            pred_ans = torch.argmax(pred_logits_qa, dim=-1).cpu().tolist()
            pred_seq = self.t5_tokenizer.decode(outputs_qa.sequences.squeeze(), skip_special_tokens=True) #MODIFIED: Used to get output sequence
        
        out['output_text'] = pred_ans
        out['output_sequence'] = pred_seq
        if 'qa_vid' not in self.task: 
            out['temp_idx'] = [j for i in range(b) for j in range(t)]
            # out['answer'] = [a for a in answer for i in range(t)]
            out['qid'] = [q for q in qid for i in range(t)]
    ...
    def predict_answers(
        ...
        **kwargs
        ...
        atts_t5 = torch.ones(inputs_t5.size()[:-1], dtype=torch.long).to(self.dev)
    ...
    def from_config(cls, cfg):
        ...
        use_vit = cfg.get("use_vit", True)
        use_optical_flow = cfg.get("use_optical_flow", False)
        total_frames = cfg.get("total_frames", 8)
        use_qformer_clf = cfg.get("use_qformer_clf", False)
        ...
        model = cls(
            ...
            use_qformer_clf=use_qformer_clf,
        )
