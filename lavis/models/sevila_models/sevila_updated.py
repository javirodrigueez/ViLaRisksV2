...
@registry.register_model("sevila_features")
class SeViLAFeatures(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        use_vit=True, device='cpu', total_frames=8):
        """
        ...
        if self.use_vit:
            ...
            self.ln_vision = nn.LayerNorm(self.num_visual_features)
            self.ln_vision_loc = nn.LayerNorm(self.num_visual_features)
        ...
        num_query_token, self.num_visual_features) # self.visual_encoder.num_features
    ...
    def forward(self, samples,
        ...
        if self.use_vit:
            ...
            image_embeds = samples['features_file'].to(self.dev)
            b = len(image_embeds)
            t = self.total_frames
            c = 3
        ...
        # finetune answerer with random frames
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                ...
                if 'qa_vid' not in self.task:
                    ...
                    # print('><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
                    # print(inputs_t5_qa.shape)
                    inputs_t5_qa = inputs_t5_qa.reshape(b, t, inputs_t5_qa.shape[-2], -1) # b, t, m ,c
                    atts_t5_qa = torch.ones(inputs_t5_qa.size()[:-1], dtype=torch.long).to(self.dev)
                    
                    # print(vid_prefix_embed.shape, inputs_t5_qa.shape)
                    inputs_t5_qa = torch.cat([vid_prefix_embed, inputs_t5_qa], dim=2) # b, t, n_word + m, c
                    atts_t5_qa = torch.cat([vid_prefix_mask, atts_t5_qa], dim=2) # b, t, n_word + m 
                    inputs_t5_qa = inputs_t5_qa.reshape(b, -1, inputs_t5_qa.shape[-1])
                    atts_t5_qa = atts_t5_qa.reshape(b, -1)
    ...
    def generate_demo(self,
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            pred_ans = torch.argmax(pred_logits_qa, dim=-1).cpu().tolist()
            pred_topk = torch.topk(pred_logits_qa, 5, dim=-1).indices.cpu().tolist()
