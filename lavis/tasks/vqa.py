...
class VideoQA(BaseTask):
    def __init__(self):
        super().__init__()
        # MODIFIED: Extended till 32 (A-g)
        #self.ANS_MAPPING = {'A':0, 'B':1, 'C':2, 'D':3, 'E':4, 'F':5, 'G':6, 'H':7, 'I':8, 'J':9, 'K':10, 'L':11, 'M':12, 'N':13, 'O':14, 'P':15, 'Q':16, 'R':17, 'S':18, 'T':19, 'U':20, 'V':21, 'W':22, 'X':23, 'Y':24, 'Z':25, 'a':26, 'b':27, 'c':28, 'd':29, 'e':30, 'f':31, 'g':32}
        # Extended till 32 (A-GG)
        ...
        #self.ANS_MAPPING = {v: k for k, v in self.ANS_MAPPING.items()}
    def valid_step(self, model, samples):
        results = []
    ...
    def _report_metrics(self, eval_result_file, split_name):
        ...
        for qtype in qtype_total_dict:
            metrics[qtype] = qtype_correct_dict.get(qtype, 0) / qtype_total_dict[qtype] * 100
