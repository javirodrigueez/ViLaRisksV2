...
class Action_DATASETS(data.Dataset):
    ...
    def _load_image(self, directory, idx):
        ...
        return [Image.open(os.path.join(directory, self.image_tmpl.format(directory.split('/')[-1],idx))).convert('RGB')]
    @property
    ...
    def _parse_list(self):
        ...
        self.video_list = []
        min_len = self.total_length * 6
        for i in range(len(video_list)):
            if video_list[i].num_frames > min_len:
                self.video_list.append(video_list[i])
    ...
    def _get_val_indices(self, record):
        if self.num_segments == 1:
            return np.array([record.num_frames //2], dtype=np.intc) + self.index_bias
        ...
        if record.num_frames <= self.total_length:
            ...
            return np.array([i * record.num_frames // self.total_length
                             for i in range(self.total_length)], dtype=np.intc) + self.index_bias
        ...
        return np.array([i * record.num_frames / self.num_segments + offset + j
                         for i in range(self.num_segments)
                         for j in range(self.seg_length)], dtype=np.intc) + self.index_bias
    
    def  _get_val_indices_temp(self, record):
        downsample_rate = 6
        offset = np.random.choice(range(record.num_frames - self.total_length*downsample_rate), 1)[0]
        start_idx = record.start + offset
        # end_idx = start_idx + self.total_length*downsample_rate
        indices = [i + self.index_bias for i in range(start_idx, start_idx + self.total_length*downsample_rate, downsample_rate)]
        return indices
    ...
    def __getitem__(self, index):
        ...
        if record.start == -1:
            ...
            segment_indices = self._sample_indices(record) if self.random_shift else self._get_val_indices_temp(record)
